...
HASH_PER_SEC = (10 ** 9) * 30   # 30 milliard
# http://hashcat.net/oclhashcat-plus/#tested-gpu
# -> With 8 GPUs this cracking speed is possible.
...
class PasswordOracle(object):
    ...
    def modify_wordlist(self):
        ...
        self.password_list += [x + '!' for x in self.password_list]

        self.log.log('Changing o -> 0 and i -> 1.')
        self.password_list = [x.replace('o', '0').replace('i', '1')
                              for x in self.password_list]

        self.log.log('Wordlist contains now %s words.' %
                     len(self.password_list))
    ...
    def count_entropy(self, charsetlen, passwordlen):
        """Counts entropy of a password based on formula found from
        http://en.wikipedia.org/wiki/Password_strength"""
        entropy = passwordlen * math.log(charsetlen) / math.log(2)
    ...
    def give_grade(self, password):
        ...
        entropy = self.count_entropy(charsetlen, len(password))

        self.log.log('Charset\'s length is %s.' % charsetlen)

        self.log.log('Entropy of password is %s bits. 2^%s = %s(entropy based combinations)' % (entropy, entropy, 2 ** entropy))

        self.log.log('1 - %s length passwords\' combinations: %s.' %
                     (len(password), all_combs))

        self.log.log('On average, half of the combinations have to be tested to crack the password.')
        self.log.log('1 - %s length\'s bruteforcing time: ~%s' %
                     (len(password), secs_readable(all_secs / 2)))
    ...
    def charset_len(self, password):
        ...
        # To determine already added sets.
        used_sets = [False] * len(charsets)
    ...
    def analyze_all(self, words):
        ...
        for word in words:
            ...
            self.log.log('Analyzing password "%s" (length = %s)' %
                         (word, len(word)))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print('Usage: ./anapass.py passwords_to_analyze.txt [wordlist1.txt] ... [wordlistN.txt]')
        sys.exit(1)
